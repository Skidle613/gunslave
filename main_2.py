...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, player, pos):
        ...
        self.add(all_sprites)
        self.player = player
        self.image = pygame.transform.scale(Player.player_image, (13, 16))
        self.rect = self.image.get_rect()
        self.rect.x = player.rect.x + player.rect.width / 2
        self.rect.y = player.rect.y + player.rect.height / 2
        self.v_x = 25 * abs((pos[0] - self.rect.x) / (abs(pos[1] - self.rect.y) + abs(pos[0] - self.rect.x) + 1))
        self.v_y = 25 - self.v_x
        self.v_x *= (pos[0] - self.rect.x) / (abs(pos[0] - self.rect.x) + 1)
        self.v_y *= (pos[1] - self.rect.y) / (abs(pos[1] - self.rect.y) + 1)

    def update(self):
        self.rect.x += self.v_x
        self.rect.y += self.v_y
        if pygame.sprite.spritecollideany(self, mobs_sprite):
            pygame.sprite.spritecollideany(self, mobs_sprite).health -= self.player.attacck
            self.kill()
...
def main_game():
    ...
    Room('i', player, way, 3)
    Room('i', player, 2, 3)
    Room('i', player, 4 - way, 3)
    Room('i', player, 4 - way, 2)
    if way == 1:
    ...
    Room('l', player, way, 3)
    Room('l', player, 2, 3)
    Room('l', player, 4 - way, 3)
    Room('l', player, 4 - way, 2)
    Room('l', player, 2, 4)
    Room('l', player, 2, 5)
    Room('l', player, 1, 5)
    if way == 1:
    ...
    Wall(player, [w // 2 - 100, h // 2 - 150, 200, 300], room_finish)
